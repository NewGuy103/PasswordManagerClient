...
class PasswordsTabController(QObject):
    ...
    def database_loaded(self, database: MainDatabase):
        self.db = database
        make_worker_thread(self.db.groups.get_children_of_root, self.setup, self.worker_exc_received)
...
class EntriesDataController(QObject):
    ...
    def delete_password_entry(self, item: PasswordEntryData):
        ...
        if self.client:
            net_func = partial(self.client.entries.delete_entry_by_id, item.entry_id, item.group_id)
    ...
    def sync_edit_password_entry(self, data: EntryPublicGet):
        ...
        func = partial(self.db.entries.update_entry_data, entry.entry_id, entry)
